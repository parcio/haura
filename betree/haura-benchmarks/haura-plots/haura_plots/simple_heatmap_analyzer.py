#!/usr/bin/env python3
"""
Simple multi-dimensional heatmap analyzer for Haura benchmark results
Creates separate heatmaps for each metric without heavy dependencies
"""
...
class SimpleHauraBenchmarkAnalyzer:
        ...
        for entry_size in self.entry_sizes:
            for compression in self.compression_algorithms:
                ...
                for thread_count in self.thread_counts:
                    ...
                    if key in self.data:
                        value = self.data[key].get(metric, None)
    ...
    def create_plotly_heatmap(self, metric: str, title: str, colorscale: str = 'Viridis', reverse_scale: bool = False) -> str:
        """Create a Plotly heatmap JSON for a specific metric"""
        ...
        heatmap_data = {
            ...
            "layout": {
                ...
                "yaxis": {"title": "Compression Algorithm & Entry Size"},
                "width": 800,
                "height": 600,
                "font": {"size": 12}
            }
...
<html>
<head>
    <title>Haura Benchmark Heatmaps (Grouped by Entry Size)</title>
...
<body>
    ...
    <div class="data-info">
        ...
        <ul>
            ...
            <li><strong>Individual Scales:</strong> Each entry size group has its own color scale for better comparison</li>
            <li><strong>Colors:</strong> Darker/brighter colors indicate better performance within each group</li>
            <li><strong>Values:</strong> Hover over cells to see exact values and configuration details</li>
            <li><strong>Comparison:</strong> Compare compression algorithms within the same entry size group</li>
        </ul>
    </div>
"""]
...
def main():
    ...
    # Create analyzer
    analyzer = SimpleHauraBenchmarkAnalyzer(args.results_dir)
