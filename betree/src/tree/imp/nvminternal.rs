...
impl<N> NVMInternalNode<N> {
    ...
    ) -> (&std::sync::Arc<std::sync::RwLock<Option<InternalNodeData<N>>>>, usize) {
        let idx = self.idx(key);
        if idx > 0 {
...
mod tests {
    ...
    use crate::{
        arbitrary::GenExt,
        database::DatasetId,
        tree::default_message_action::{DefaultMessageAction, DefaultMessageActionMsg}, data_management::Object,
    };
    use bincode::serialized_size;
    ...
    impl<T: Clone> Clone for NVMInternalNode<T> {
        fn clone(&self) -> Self {
            NVMInternalNode {
                ...
                data: std::sync::Arc::new(std::sync::RwLock::new(Some(InternalNodeData {
                    children: self.data.read().as_ref().unwrap().as_ref().unwrap().children.to_vec(),
                }))),
                meta_data_size: 0,
                data_size: 0,
                data_start: 0,
    ...
    impl<T: Arbitrary + StaticSize> Arbitrary for NVMInternalNode<T> {
        fn arbitrary(g: &mut Gen) -> Self {
            ...
            let mut children: Vec<Option<NVMChildBuffer<T>>> = Vec::with_capacity(pivot_key_cnt + 1);
    ...
    fn check_size<T: Size + ObjectReference + std::cmp::PartialEq>(node: &mut NVMInternalNode<T>) {
        ...
        let mut serializer_meta_data = rkyv::ser::serializers::AllocSerializer::<0>::default();
        serializer_meta_data.serialize_value(&node.meta_data).unwrap();
        let bytes_meta_data = serializer_meta_data.into_serializer().into_inner();

        let mut serializer_data = rkyv::ser::serializers::AllocSerializer::<0>::default();
        serializer_data.serialize_value(node.data.read().as_ref().unwrap().as_ref().unwrap()).unwrap();
        let bytes_data = serializer_data.into_serializer().into_inner();

        let archivedinternalnodemetadata: &ArchivedInternalNodeMetaData = rkyv::check_archived_root::<InternalNodeMetaData>(&bytes_meta_data).unwrap();
        let meta_data: InternalNodeMetaData = archivedinternalnodemetadata.deserialize(&mut rkyv::de::deserializers::SharedDeserializeMap::new()).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e)).unwrap();

        let archivedinternalnodedata: &ArchivedInternalNodeData<_> = rkyv::check_archived_root::<InternalNodeData<T>>(&bytes_data).unwrap();
        let data: InternalNodeData<_> = archivedinternalnodedata.deserialize(&mut rkyv::de::deserializers::SharedDeserializeMap::new()).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e)).unwrap();
    ...
    fn check_insert_msg_buffer(
        ...
        for (Key(key), (keyinfo, msg)) in buffer {
            ...
            added_size_twin +=
                node_twin.data.write().as_mut().unwrap().as_mut().unwrap().children[idx].as_mut().unwrap().insert(key, keyinfo, msg.0, DefaultMessageAction);
        }
        ...
        assert_eq!(node.data.read().as_ref().unwrap().as_ref().unwrap(), node_twin.data.read().as_ref().unwrap().as_ref().unwrap());
