    ...
    I: Borrow<Inner<X::ObjectRef, M>>,
{
    ...
    pub(super) fn rebalance_tree(
        ...
        loop {
            if !node.is_too_large(self.storage_map, self.storage_default) {
                return Ok(());
            }
            debug!(
                "{}, {:?}, lvl: {}, size: {}, actual: {:?}",
            ...
            if !child.is_leaf() && child.is_too_large(self.storage_map, self.storage_default) {
            ...
            while child.is_too_large_leaf(self.storage_map, self.storage_default) {
            ...
            if child_buffer.size() > super::MAX_INTERNAL_NODE_SIZE {
                ...
                if child.is_too_large(self.storage_map, self.storage_default) {
                    warn!("... but child, too");
                }
                node = child_buffer.into_owner();
                continue;
