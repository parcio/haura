 ...
 */
static void hm_tx_rebuild(PMEMobjpool *pop, TOID(struct hashmap_tx) hashmap,
                          size_t new_len) {
  ...
  TX_BEGIN(pop) {
    ...
    for (size_t i = 0; i < D_RO(buckets_old)->nbuckets; ++i) {
      while (!TOID_IS_NULL(D_RO(buckets_old)->bucket[i])) {
        ...
        uint64_t h = hash(&hashmap, &buckets_new, D_RO(en)->key);
...
int hm_tx_insert(PMEMobjpool *pop, TOID(struct hashmap_tx) hashmap,
  ...
  for (var = D_RO(buckets)->bucket[h]; !TOID_IS_NULL(var);
       var = D_RO(var)->next) {
    if (D_RO(var)->key == key)
      return 1;
    num++;
...
/*
** Cast a PMem ptr to an actual usable struct value.
*/
...
int root_needs_init(struct root *r) { return TOID_IS_NULL(r->map); }
